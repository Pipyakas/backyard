...
use crate::state::{Library, Model, State as AppState};
use crate::config::AppConfig;

const FRONTEND_DIST: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../frontend/dist");
const FRONTEND_STATIC: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../frontend/static");
...
pub async fn start_server(app_state: Arc<AppState>, config: Arc<Mutex<AppConfig>>) -> Result<()> {
    let server_state = Arc::new(ServerState { app_state, config });
    ...
    let app = Router::new()
        ...
        .route(
            "/api/benchmarks",
            post(run_benchmark_api).get(list_benchmarks_stub),
        )
        ...
        .nest_service("/static", ServeDir::new(FRONTEND_STATIC))
        .fallback_service(ServeDir::new(format!("{}/templates", FRONTEND_DIST)))
        .with_state(server_state);

    let listener = tokio::net::TcpListener::bind("127.0.0.1:5556").await?;
    eprintln!("Server listening on http://127.0.0.1:5556");
...
async fn serve_index() -> impl IntoResponse {
    let index_path = format!("{}/templates/index.html", FRONTEND_DIST);
...
async fn list_libraries(State(state): State<Arc<ServerState>>) -> Json<Vec<Library>> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, path FROM libraries")
        .unwrap();
    let libs = stmt
        .query_map([], |row| {
            Ok(Library {
...
async fn reset_db_api(State(state): State<Arc<ServerState>>) -> impl IntoResponse {
    ...
    let default_path = std::path::PathBuf::from(home_dir)
        .join(".backyard")
        .join("models");
...
async fn download_model_api(
    ...
    let default_path = {
        let conn = state.app_state.conn.lock().unwrap();
        conn.query_row(
            "SELECT path FROM libraries WHERE name = 'Default'",
            [],
...
async fn move_model_api(
    ...
    crate::state::fs::move_model(&state.app_state, model_id, target_lib)
        .await
        .unwrap();
    "Model moved"
}
...
async fn start_engine_api(
    ...
    let container_id = orchestrator
        .start_engine(req.model_id, &state.app_state, &req.gpu_type)
        .await
        .unwrap();
    Json(container_id)
}
...
async fn run_benchmark_api(
    ...
    if output.status.success() {
        ...
        format!("Benchmark failed: {}", stderr)
    }
}
...
async fn list_engines_api() -> Json<Vec<EngineInfo>> {
    ...
    for line in stdout.lines() {
        ...
        if parts.len() >= 4 {
            ...
            let status = if ps_stdout.is_empty() {
                "stopped".to_string()
            } else {
                ps_stdout.trim().to_string()
            };
...
async fn get_config(State(state): State<Arc<ServerState>>) -> Json<AppConfig> {
...
async fn set_config(
    ...
    cfg.close_to_tray = req.close_to_tray;
